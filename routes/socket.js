...
module.exports = function (io, app) {
  return function (socket) {
    ...
    socket.on('buzzers:arm', function (data) {
      io.emit('buzzers:arm', data);
    });

    socket.on('buzzer:verdict', function (data) {
      console.log('SERVER RECEIVED buzzer:verdict ->', JSON.stringify(data));
      io.emit('buzzer:verdict', data);
    });

    socket.on('board:verdict', function (data) {
      console.log('SERVER RECEIVED board:verdict ->', JSON.stringify(data));
      io.emit('board:verdict', data);
    });
  };
};
